...
//            strBuilder.append(" ");
    ...
    //字节数组转转字符串列表
    public static List<String> ByteArrToHexArray(byte[] inBytArr) {
        List<String> arrayList = new ArrayList<>(inBytArr.length);
        for (byte valueOf : inBytArr) {
            arrayList.add(Byte2Hex(valueOf));
//            strBuilder.append(" ");
        }
        return arrayList;
    }
    ...
    public static String ByteArrToInt(byte[] inBytArr, int offset, int byteCount) {
        ...
        int j = byteCount;
        for (int i = offset; i < j; i++) {
            String byte2Hex = Byte2Hex(Byte.valueOf(inBytArr[i]));
//            int decodeHEX = decodeHEX(byte2Hex);
            strBuilder.append(byte2Hex);
        }
        long decodeHEX = Long.parseLong(strBuilder.toString(),16);
        return decodeHEX + "";
    }
    ...
    public static String ByteArrToInt(byte[] inBytArr) {
        StringBuilder strBuilder = new StringBuilder();
        for (byte valueOf : inBytArr) {
            String byte2Hex = Byte2Hex(Byte.valueOf(valueOf));
            int decodeHEX = decodeHEX(byte2Hex);
    ...
    public static String byteArray2String(byte[] readData){
//        byte[] newByte = new byte[size];
//        for (int i = 0; i < size; i++) {
//            newByte[i] = readData[i];
//        }
